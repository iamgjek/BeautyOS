  ...
  <C-Main-Block>
    ...
    <section class="relative p-4">
      <div class="relative mb-4 flex">
        <font-awesome-icon icon="fa-regular fa-list" class="absolute left-4 text-xl"
          @click="$router.push({ name: 'Order-list' })" @keypress="$router.push({ name: 'Order-list' })" />
        <h1 class="m-auto font-bold">預約總覽</h1>
        <font-awesome-icon icon="fa-regular fa-circle-plus" class="absolute right-4 text-xl"
          @click="$router.push({ name: 'ScheduleBreak-Add' })" @keypress="$router.push({ name: 'ScheduleBreak-Add' })" />
      </div>
      <!-- locale="zh-TW" -->
      <transition name="collapseTransition">
        <vc-date-picker locale="en" @update:to-page="swipeMonth($event)" :select-attribute="selectedAttribute" is-expanded
          ...
          <template v-slot:header-left-button>
            <font-awesome-icon @click="preNextMonth(-1)" icon="fa-solid fa-angle-left" class="my-3" />
          </template>
          <!-- 右箭頭 -->
          <template v-slot:header-right-button>
            <font-awesome-icon @click="preNextMonth(1)" icon="fa-solid fa-angle-right" class="my-3" />
          </template>
          ...
          <template v-slot:day-content="{ day, attributes }">
            ...
            <div class="flex flex-col" @click="setSelectDate(day.date)"
              ...
              <div class="text-[#231815]">
                <p class="day-label relative my-2 flex flex-col text-sm" :class="{'before:contents-[``] !text-white before:absolute  before:inset-x-0 before:top-[-1px] before:z-[-1] before:m-auto before:inline-block before:h-[24px] before:w-[24px] before:rounded-[50%] before:bg-[#F6BA7B]': $dayjs(selectedDate).valueOf()===$dayjs(day.date).startOf('d').valueOf()}">
              ...
              <div class="m-auto inline-block h-6">
                ...
                <span v-if="hasOnlyOrder(attributes)"
                  class="m-[2px] inline-block h-[5px] w-[5px] rounded-[50%] bg-[#F6BA7B]"></span>
              </div>
...
/* 預約明細類 */
.sect h3 {
  @apply text-left ml-8 relative text-[#231815] font-[500]  before:absolute before:top-[5px] before:left-[-8px] before:w-[3px] before:h-[70%] before:bg-[#231815] before:rounded-[5px];
}
.sect h3::before{
  content: '';
}
...
.sect .content {
  @apply mb-2 select-none text-[13px] font-[500] text-[#231815] mx-4 my-2 flex items-center justify-between rounded-lg bg-[#F7F7F7] p-2;
}
