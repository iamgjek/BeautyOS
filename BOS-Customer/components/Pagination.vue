  ...
  <section>
    <ul v-if="totalPage > 1" class="flex justify-center items-center gap-1 mt-8 select-none">
      <li @click="currentPage === 1 ? currentPage = 1 : currentPage -= 1, currentPage % 5 === 0 ? currentPageTab -= 1 : currentPageTab = currentPageTab, changePage()" class="group px-3 py-2 rounded-full cursor-pointer-[#232D4E]-0-white flex justify-center items-center" :class="{'pointer-events-none': currentPage === 1}">
        <fa class="text-lg:text-white" :class="{'text-[#a3a3a3]': currentPage === 1, 'text-[#232D4E]': currentPage !== 1}" icon="fa-solid fa-angle-left" />
      </li>
      ...
      <template v-else>
        <li @click="currentPageTab -= 1, currentPage = pageTab[currentPageTab][4], changePage()" class="cursor-pointer px-3 py-2 rounded-full" :class="{'hidden': currentPageTab === 0 }"><fa icon="fa-regular fa-ellipsis" /></li>
        <li v-for="page in pageTab[currentPageTab]" :key="page" @click="currentPage = page, changePage()" class="w-9 h-9 flex justify-center items-center py-2 rounded-full cursor-pointer" :class="{'bg-gradient-to-t from-[#5D7A9D] to-[#094194] text-white ring-[#232D4E]': currentPage === page}">{{ page }}</li>
        <li @click="currentPageTab += 1, currentPage = pageTab[currentPageTab][0], changePage()" class="cursor-pointer px-3 py-2 rounded-full" :class="{'hidden': currentPageTab === (pageTab.length - 1) }"><fa icon="fa-regular fa-ellipsis" /></li>
      </template>
      <li @click="currentPage === totalPage ? currentPage = totalPage : currentPage += 1, currentPage % 5 === 1 ? currentPageTab += 1 : currentPageTab = currentPageTab, changePage()" class="group px-3 py-2 rounded-full cursor-pointer-[#232D4E]-0-white" :class="{'pointer-events-none': currentPage === totalPage}">
        <fa class="text-lg:text-white" :class="{'text-[#a3a3a3]': currentPage === totalPage, 'text-[#232D4E]': currentPage !== totalPage}" icon="fa-solid fa-angle-right" />
      </li>
    </ul>
  </section>
</template>
