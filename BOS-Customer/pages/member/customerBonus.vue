<template>
  <section id="firstPos">
    <div v-once class="fixed left-0 top-0 w-full px-[10%] text-lg bg-white py-4 flex items-center">
      ...
      <h2 v-pre class="absolute top-1/2 translate-y-[-50%] left-[50%] translate-x-[-50%] text-lg font-bold">紅利點數</h2>
    </div>
    ...
    <div v-if="merchants.length > 0 && !isLoading" data-aos="fade-up" data-aos-duration="300" data-aos-once="true" data-aos-delay="100" data-aos-anchor="#firstPos" class="text-center mb-5 pb-5 relative after:content-[''] after:w-screen after:h-[1px] after:bg-[#232D4E] after:absolute after:left-[-6%] md:after:left-[-1.5%] after:bottom-0">
      <div class="w-full relative mb-5">
        <select @change="getBonusRecords()" v-if="merchants.length > 0" v-model="selectMerchantId" class="w-full p-3 text-center rounded-lg shadow-[0px_2px_2px_rgba(0,0,0,0.25)] bg-white appearance-none">
      ...
      <p v-pre class="text-sm mt-2">剩餘紅利點數</p>
    </div>

    <!-- 紅利列表 -->
    <ul v-once v-if="addValueRecords.length > 0 && !isLoading" data-aos="fade-up" data-aos-duration="400" data-aos-once="true" data-aos-delay="200" data-aos-anchor="#firstPos" class="bg-white rounded-xl shadow-md text-sm">
      <li v-for="(item, idx) in addValueRecords" :key="item.id" data-aos="fade" :data-aos-delay="500 + (idx * 100)" data-aos-once="true" data-aos-duration="400" data-aos-anchor="#firstPos" class="px-6 py-5 border-b border-[#aeaeae]" :class="{'border-b-0': (idx + 1) === addValueRecords.length}">
        <div class="grid grid-cols-12 gap-4">
          ...
          <div class="col-span-7 flex gap-1" :class="{'text-[#16A34A]': item.type === '獲得', 'text-[#F43F5E]': item.type !== '獲得'}">
          ...
          <div v-if="item.generationWay" class="col-span-5"><p>產生方式</p></div>
          <div v-if="item.generationWay" class="col-span-7"><p>{{ item.generationWay }}</p></div>

          <div v-if="item.remark" class="col-span-5"><p>備註</p></div>
          <div v-if="item.remark" class="col-span-7"><p>{{ item.remark }}</p></div>
        </div>
      </li>
    </ul>
