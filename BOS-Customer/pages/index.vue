<template>
  <section>
    <client-only v-if="!isLoading">
      ...
      <div class="flex flex-col w-full px-[10%] h-[100vh] overflow-x-hidden overflow-y-hidden gap-10 justify-center items-center bg-[#f1f1f1]">
        <h2 v-pre data-aos="fade-up" data-aos-once="true" data-aos-delay="100" data-aos-duration="700" class="text-black text-lg">
          會員登入
        </h2>
        <div data-aos="fade-up" data-aos-once="true" data-aos-delay="250" data-aos-duration="700" class="flex flex-col w-full max-w-[450px] justify-start">
          <ValidationObserver v-slot="{ handleSubmit }">
            <form @submit.prevent="handleSubmit(login)">
              <label v-pre for="phoneInput" class="text-black text-sm">手機：</label>
              <div class="form-group relative mb-10 mt-2">
                <ValidationProvider name="手機" rules="required|onlyNumber" v-slot="{ errors }" class="w-full">
                  <input ref="phoneRef" v-model="member.phone" onpaste="return false" maxlength="10" id="phoneInput" type="tel" class="w-full p-2.5 rounded-xl focus:outline-none border border-[#aeaeae]" :class="{'ring-2 ring-[#ff4444]': errors.length > 0}">
                  <span v-if="errors.length > 0" class="absolute left-3 md:left-10 -bottom-5 text-red-500 text-xs font-bold">{{ errors[0] }}</span>
                </ValidationProvider>
              </div>

              <label v-pre for="verifyInput" class="text-black text-sm sm:text-black">驗證碼：</label>
              <div class="form-group relative mt-2">
                <ValidationProvider name="驗證碼" rules="required|onlyNumber" v-slot="{ errors }" class="w-full">
                  <input v-model="member.verifyCode" minlength="5" maxlength="5" id="verifyInput" type="tel" class="w-full p-2.5 rounded-xl focus:outline-none border border-[#aeaeae]" :class="{'ring-2 ring-[#ff4444]': errors.length > 0}">
                  <span v-if="errors.length > 0" class="absolute left-3 md:left-10 -bottom-5 text-red-500 text-xs font-bold">{{ errors[0] }}</span>
                  <button @click="getVerifyCode" type="button" :disabled="getCodeWaitingTimer || getingCode" :class="{'bg-[#FEB401]': !getCodeWaitingTimer, 'bg-[#bebebe]': getCodeWaitingTimer || getingCode}" class="absolute right-0 bottom-0 h-full p-2.5 text-white text-sm rounded-[0_12px_12px_0] flex items-center gap-1">
                    <fa v-if="getingCode" icon="fa-solid fa-spinner" class="animate-spin-slow" />
                    取得驗證碼
                    <p v-if="getCodeWaitingTimer">({{ getCodeWaitingTimer }})</p>
                  </button>
                </ValidationProvider>
              </div>
              <button :disabled="isLogin" type="submit" class="w-full py-3 text-white text-sm mt-10 bg-[#2f3549] rounded-xl shadow-md flex justify-center items-center gap-3 duration-300" :class="{'bg-[#555555]': isLogin}">
                <fa v-if="isLogin" icon="fa-solid fa-spinner" class="animate-spin-slow" />
                登入
              </button>
            </form>
          </ValidationObserver>
